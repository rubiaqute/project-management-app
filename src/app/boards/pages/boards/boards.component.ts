...
export class BoardsComponent implements OnInit {
  ...
  constructor(private apiFacade: ApiFacade, public dialog: MatDialog, private apiService: ApiServices) {
  ...
  getTasks(boards: IBoard[]): Observable<ITaskSearch[]> {
    ...
    boards.forEach((el) => {
      observables.push(
        this.apiService.getBoardById(el.id!)
          .pipe(
            map((data) => {
              let tasks: ITask[] = []
  ...
  getFilteredTasks(tasks: ITask[]) {
    return tasks.filter((task) => {
      return task.title.includes(this.taskSearchInput) ||
        task.description.includes(this.taskSearchInput) ||
        this.getAssigneeName(task.userId) && this.getAssigneeName(task.userId).includes(this.taskSearchInput)
    })
  ...
  getAssigneeName(userId: string) {
    if (this.users.length) {
      return this.users.filter((el) => el.id === userId)[0]?.name || 'was deleted from base'
    } return ''
  }
  search(value: string) {
    this.title = value;
